 ...
 * be more — could otherwise write the Authorization header into the log. Redacting
 * at the client keeps the credential out of any error object before a logger sees it.
 *
 ...
 * forgotten; scrubbing the object means the credential is not present to be logged.
 *
 ...
 * scrubbing the object means the credential is not present to be logged.
 *
 * Both the error AND `err.response` are sanitised: the response holds its own copies of
 * config and request, which is the second place the header can be reached.
 * `pveMessage()` remains the right thing to log; this makes the careless path safe too.
 */
...
export async function getLxcIps(node: string, vmid: number, client?: AxiosInstance): Promise<GuestIps> {
  ...
  try {
    ...
    for (const iface of res.data.data ?? []) {
      ...
      // `inet` is a CIDR like "192.168.1.40/24"; strip the prefix.
...
/** Compute the network address (e.g. 192.168.1.122/24 → 192.168.1.0/24) for an IPv4 CIDR. */
