...
describe('getLxcIps', () => {
  it('returns the first non-loopback IPv4 as the LAN IP, stripping the CIDR prefix', async () => {
    ...
    c.get.mockResolvedValue({
      data: { data: [{ name: 'lo', inet: '127.0.0.1/8' }, { name: 'eth0', inet: '192.168.1.40/24' }] },
    });
    expect(await getLxcIps(NODE, VMID, asClient(c))).toEqual({ ip: '192.168.1.40', tailscaleIp: null });
    ...
    c.get.mockResolvedValue({
      data: {
        data: [
          ...
          { name: 'eth0', inet: '192.168.1.40/24' },
        ],
      },
    });
