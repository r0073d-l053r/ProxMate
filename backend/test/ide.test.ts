...
describe('ide_ingress_cidr (admin-visible pinhole source)', () => {
  it('defaults to empty and round-trips through save/get (trimmed)', async () => {
    ...
    await saveIdeConfig({ enabled: 'tenants', allowByoKeys: false, ingressCidr: ' 192.168.1.228/32 ' });
    expect((await getIdeConfig()).ingressCidr).toBe('192.168.1.228/32');
  });
  ...
  it('isValidIngressCidr accepts real IPv4 CIDRs and the empty clear-value', () => {
    for (const ok of ['', '  ', '192.168.1.228/32', '10.0.0.0/8', '0.0.0.0/0', '172.16.4.1/26']) {
  ...
  it('isValidIngressCidr rejects malformed values', () => {
    for (const bad of [
      '192.168.1.228', // no prefix
      '192.168.1.228/33', // prefix out of range
