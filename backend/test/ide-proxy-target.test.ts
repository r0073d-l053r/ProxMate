...
const vmA = { ipAddress: '192.168.1.222' };
const vmB = { ipAddress: '192.168.1.40' };
...
describe('resolveIdeTargetUrl', () => {
  ...
  it('targets the guest LAN IP on port 8080 by default', () => {
    expect(resolveIdeTargetUrl(vmA, {})).toBe('http://192.168.1.222:8080');
  });

  it('honours IDE_GUEST_PORT', () => {
    expect(resolveIdeTargetUrl(vmA, { IDE_GUEST_PORT: '9000' })).toBe('http://192.168.1.222:9000');
  });
