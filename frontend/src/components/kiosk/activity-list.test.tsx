...
describe("KioskActivityList", () => {
  it("renders every entry with detail and IP", () => {
    const { container } = render(<KioskActivityList audit={AUDIT} total={42} />);
    expect(container.textContent).toContain("web-server-01 started");
    expect(container.textContent).toContain("db-box stopped");
  ...
  it("filters by IP on tap and clears again", () => {
    ...
    expect(container.textContent).toContain("web-server-01 started");
    expect(container.textContent).not.toContain("db-box stopped");
  ...
  it("filters by VM (stable targetId) via the VM chip", () => {
    ...
    expect(container.textContent).toContain("db-box stopped");
    expect(container.textContent).not.toContain("web-server-01 started");
